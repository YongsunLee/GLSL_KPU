...
void Renderer::Initialize(int windowSizeX, int windowSizeY)
{
	...
	m_CubeShader = CompileShaders("./Shaders/ProjectionShader.vs", "./Shaders/ProjectionShader.fs");
	m_PerspectiveCubeShader = CompileShaders("./Shaders/PerspectiveProjectionShader.vs", "./Shaders/PerspectiveProjectionShader.fs");
	...
	m_v3Camera_Lookat = glm::vec3(0.f, 0.f, 0.f);		// 바라보는 지점
	...
	m_m4OrthoProj = glm::ortho(-1.f, 1.f, -1.f, 1.f, 0.f, 2.f);
		// Depth 해상도에서 결과가 좋지 않을 수 있으므로 Near 와 Far의 값을 적절히 주는 것이 좋다.

	m_m4PerspectiveProj = glm::perspectiveRH(45.f, (float)500.f/(float)500.f, 0.f, 10.f);

	// Projection-View Matrix
	m_m4ProjView = m_m4OrthoProj * m_m4View;
	m_m4PerspectiveView = m_m4PerspectiveProj * m_m4View;

	// Init transForm
	m_v3ModelTranslation = glm::vec3(0.f, 0.f, 0.f);
	m_v3ModelRotation = glm::vec3(3.14 / 4.f, 3.14 / 4.f, 0.f);		// 오일러 각 축별로 얼마만큼 회전했는가
	m_v3ModelScaling = glm::vec3(1.f, 1.f, 1.f);		// 각 축별로 얼마만큼 Scaling 하는가
	...
	// Scale 을 곱하고 Rotate를 곱하고 Trans를 곱한다.
	// Trans * Rot * Scale * (모델이 원점이 아닐경우 원점으로 옮긴다.) * Pos 
	// 이 Matrix를 모델 Matrix라고 한다.
...
{
	DrawTexture(m_Tex_twice, 100, 100, 150, 150);
}
...
void Renderer::DrawPerspectiveCube()
{
	GLuint shader = m_PerspectiveCubeShader;
	...
	GLuint projView = glGetUniformLocation(shader, "u_ProjView");
	glUniformMatrix4fv(projView, 1, GL_FALSE, &m_m4PerspectiveView[0][0]);

	GLuint ModelMatrix = glGetUniformLocation(shader, "u_Model");
	glUniformMatrix4fv(ModelMatrix, 1, GL_FALSE, &m_m4Model[0][0]);

	GLuint ModelRotateMatrix = glGetUniformLocation(shader, "u_Rotation");
	glUniformMatrix4fv(ModelRotateMatrix, 1, GL_FALSE, &m_m4ModelRotation[0][0]);
	...
	glUniform3f(LightPos, 2.f, 1.f, 0.f);
...
void Renderer::RotateObject(float x, float y)
{
	if (x > 0.0f) {
		m_v3ModelRotation += glm::vec3(5.f, 0.f, 0.f);
	}
	else if (x < 0.0f) {
		m_v3ModelRotation += glm::vec3(-5.f, 0.f, 0.f);
	}
	...
	if (y > 0.0f) {
		m_v3ModelRotation += glm::vec3(0.f, 5.f, 0.f);
	}
	else if (y < 0.0f) {
		m_v3ModelRotation += glm::vec3(0.f, -5.f, 0.f);
	}

	m_v3ModelRotation = glm::vec3(0.f, 0.f, 0.f);		// 오일러 각 축별로 얼마만큼 회전했는가
...
void Renderer::SingleTextureAniVBO()
{
